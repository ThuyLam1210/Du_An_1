...
public class ExportExcel {
    ...
    public void exportExcel(String title, String sheetName, JTable tbl, int[] colWidth) {
        try {
            ...
            Sheet sheet = wb.createSheet(sheetName);//đặt tên sheet
            sheet.addMergedRegion(new CellRangeAddress(0, 0, 0, tbl.getColumnCount() - 1));
            for (int i = 0; i < tbl.getColumnCount(); i++) {
                sheet.setColumnWidth(i, colWidth[i]);//set độ rộng
            }
            ...
            // set font, cỡ chữ, in đậm, màu chữ Column Name (hàng đầu tiên của bảng)
